...
argument is given. Each theoretical result is paired with a numerical validation in the
accompanying \texttt{validate.py} script.
\end{abstract}
...
\[
\Q(A) = \int_A Z\,\dd\P = \E^\P[Z\mathbb{1}_A] \qquad \forall A \in \F.
\]
\end{definition}
...
$B$ with $\P(B)>0$, then $\Q(B) = \E^\P[Z\mathbb{1}_B] < 0$, a contradiction; hence $Z \ge 0$ a.s.
Taking $A = \Omega$: $1 = \Q(\Omega) = \E^\P[Z\mathbb{1}_\Omega] = \E^\P[Z]$.
\end{proof}
